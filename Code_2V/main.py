...
# Coefficients in the expansion will use two variables in random space y1 and y2.
...
def c(y1,y2):
     return (y1+y2) #This data corresponds to periodic problem. See reference 

def test_dg(x):
     return np.sin(x) #math.cos(x)
#Define problem initial data.
def initial_condition(x,y1,y2):
     return np.cos(x) #This initial data corresponds to periodic problem. See reference 
#############################
def exact_solution(x,y1,y2,t):
     return np.cos(x+(y1+y2)*t)
#############################
...
#Random variable y probabiliy space \Omega=(-1,1)^2 with uniform probability distribution. 
y1_left=-1.0
y1_right=1.0
y2_left=-1.0
y2_right=1.0
...
def rho(y1,y2): #uniform distribution in \Omega=(-1.0,1.0)
     return 1.0/4.0
