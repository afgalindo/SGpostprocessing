    ...
    def output_file(self,chaos_coeff,xx_cut,i_cut, yy_cut,lim_x,lim_y): # take in list of coefficients U
        ...
        with open(expectation, 'w') as f_exp, open(variation, 'w') as f_var:
            for i in range(self.mesh.N_x):
                for kx in range(lim_x):
                    xx = self.mesh.x[i] + 0.5 * self.mesh.dx * points_x[kx]  # Compute x coordinate
                    q = [self.evaluate(chaos_coeff[k][i], points_x[kx]) for k in range(self.N_Chaos_basis)]
                    v = np.dot(self.S, q)
                
                    # Compute mean error
                    mean_error = np.cos(xx) * np.sin(1.0)**2  - v[0]
                    #mean_error  = v[0]
                    f_exp.write(f"{xx} {mean_error}\n")
                
                    # Compute variance
                    variance = sum(v[k] * v[k] for k in range(1, self.N_Chaos_basis))
                    variance_error = (0.5+(1.0/8.0)*np.cos(2.0*xx)*np.sin(2)**2-(np.cos(xx)**2)* np.sin(1.0)**4 )  - variance
    ...
    def plot_from_file(self,xx_cut,yy_cut):
        ...
        with open('expectation.txt', 'r') as file:
            for line in file:
                ...
                T.append(values[0])
                Y.append(values[1])
        #Y_exact = [np.cos(t) * np.sin(1.0)**2 for t in T] # Exact solution for comparison
        Tpp, Ypp = [], []
        ...
        #plt.plot(T, Y_exact, label='Exact solution', linestyle='--', color='green')
       ...
       #Y_var_exact = [(0.5+(1.0/8.0)*np.cos(2.0*t)*np.sin(2)**2-(np.cos(t)**2)* np.sin(1.0)**4 ) for t in T_var]  # Exact solution for comparison
        Tpp_var, Ypp_var = [], []
        ...
        #plt.plot(T_var, Y_var_exact, label='Exact solution', linestyle='--', color='green')
