...
class SGSolver:
    def __init__(self,dg,chaos,Random_Coefficient,Initial_Data,T):
        ...
        self.N_Chaos_basis=(self.N_Chaos+1)**2
        ...
        self.N_Chaos_basis = (self.N_Chaos+1)**2
        self.S = np.zeros((self.N_Chaos_basis, self.N_Chaos_basis))  #Matrix S of the diagonalization SDS^(-1)=A with the coefficients of the system of PDE's dv/dt=A*dv/dx.
        self.S_inv = np.zeros((self.N_Chaos_basis, self.N_Chaos_basis))  #Matrix S of the diagonalization SDS^(-1)=A with the coefficients of the system of PDE's dv/dt=A*dv/dx.
        self.D=np.zeros((self.N_Chaos_basis))               #Array with the eigenvalues of the diagonilazation.
...
#Set up the PDE for the coefficients by creating S, and D.
    def Set_PDE(self):
        ...
        print("Eigenvalues of the diagonalization:", self.D)
        D_abs=np.abs(self.D)
...
#Given N, it creates N arrays which will contain the DG coefficients of the ith Chaos_expansion coefficient.
    def Create_Coefficients(self):
        for i in range(self.N_Chaos_basis):
...
#  Compute the projected initial condition at a given entry
    def Initial_Condition(self,entry,xx):
        ...
        #q_entry=0.0
        # for n1 in range(self.N_Chaos+1):
        #     for n2 in range(self.N_Chaos+1):
        #         n=n1*(self.N_Chaos+1)+n2
        #         #Compute the entry q_entry
        #         q_entry+=self.S_inv[entry][n]*v_initial[n]
        q_entry = np.dot(self.S_inv[entry,:], v_initial)
        return q_entry
...
#  Solver given initial data and T
    def Solve_SG(self):
        ...
        #square=0.0        
        max=0.0
        for entry in range(self.N_Chaos_basis):
