...
class ChaosExpansion:
    ...
    #This projection will work on functions that depend only on y and t. 
    def Chaos_Galerkin_Projection_Time(self, function,tt):
        projected_f=np.zeros((self.N+1))
        for m in range(self.N+1):
            projected_f[m]=0.0
            for point in range(self.Number_Of_Quadrature_Points):
                yy=self.gp[point]
                ww=self.wp[point]
                projected_f[m]+=function(yy,tt)* self.chaos_basis_element(m,yy) * self.rho(yy) * ww
