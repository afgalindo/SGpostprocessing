...
class StochasticPP:
    ...
    def output(self,i_cut,ep_cut,yy_cut):
        print("Outputting post-processed data...")
        coeff = self.sg.Chaos_Coefficients_Extended 
...
#         plt.savefig(f'pp_cut_x_{xx}.png')
    ...
    def _output_cut_y(self, x, PP_q,yy_cut):  
        ...
        T, Y = np.loadtxt(filename, unpack=True)
        Y_exact = [self.exact_solution(t, yy_cut, self.T) for t in T]
        plt.figure(figsize=(8, 8))
        plt.plot(T, Y)
    ...
    def _output_expectation_and_variation(self, x, PP_q):
        print("Computing expectation and variation...")
